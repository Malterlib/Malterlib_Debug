...
def fg_GetAtomicIntValue(_Atomic):
	...
	ValueInt = Value.GetValueAsSigned()
	if type(ValueInt) is int:
		return ValueInt
	return None
...
class CSynthProvider_TCSharedPointer(CSynthProvider_Common):
	def __init__(self, _ValueObject, _Dictionary):
		...
		self.m_bHasRefCount = False
		self.m_bHasWeakRefCount = False
		self.m_Value = None
	...
	def update(self):
		...
		try:
			...
			fg_PrecacheType(self.m_DataType)
			if self.m_Value.GetValueAsUnsigned() != 0:
				self.m_RefCount = self.m_Value.GetValueForExpressionPath('->m_RefCount.m_RefCount')
				self.m_WeakRefCount = self.m_Value.GetValueForExpressionPath('->m_RefCount.m_WeakRefCount')
				self.m_bHasRefCount = fg_IsValidSBValue(self.m_RefCount) and fg_GetAtomicIntValue(self.m_RefCount) is not None
				self.m_bHasWeakRefCount = fg_IsValidSBValue(self.m_WeakRefCount) and fg_GetAtomicIntValue(self.m_WeakRefCount) is not None
	...
	def fp_GetChildIndex(self, _Name):
		if _Name == '[Value]':
			return self.m_NumExtraChildren
		if self.m_bHasRefCount and _Name == '[Count]':
			return self.m_NumExtraChildren + 1
		if self.m_bHasWeakRefCount and _Name == '[WeakCount]':
			return self.m_NumExtraChildren + 1 + int(self.m_bHasRefCount)
		return CSynthProvider_Common.fp_GetChildIndex(self, _Name)

	def fp_GetChildAtIndex(self, _iChild):
		if _iChild == self.m_NumExtraChildren:
			return fg_CreateDynamicValue(self.m_ValueObject, '[Value]', fg_GetValueAddress(self.m_Value), self.m_DataType)
		elif self.m_bHasRefCount and _iChild == self.m_NumExtraChildren + 1:
			Data = lldb.SBData.CreateDataFromSInt64Array(self.m_Endianness, self.m_PointerSize, [int(1 + fg_GetAtomicIntValue(self.m_RefCount))])
			return self.m_ValueObject.CreateValueFromData("[Count]", Data, self.m_CountType)
		elif self.m_bHasWeakRefCount and _iChild == self.m_NumExtraChildren + 1 + int(self.m_bHasRefCount):
			Data = lldb.SBData.CreateDataFromSInt64Array(self.m_Endianness, self.m_PointerSize, [int(fg_GetAtomicIntValue(self.m_WeakRefCount))])
	...
	def fp_NumChildren(self):
		return 1 + int(self.m_bHasRefCount) + int(self.m_bHasWeakRefCount) + self.m_NumExtraChildren
...
def fg_SummaryProvider_Pointer(_Value, dict):
	try:
		...
		if Current.GetType().IsPointerType():
			PointerValue = fg_GetValueAsUnsigned(Current)
		else:
...
def fg_SummaryProvider_TCSharedPointer(_Value, dict):
	try:
		...
		fg_PrintException()
		fg_PrintError('(fg_SummaryProvider_TCSharedPointer) error: ', error, ' path: ', _Value.get_expr_path())
		return
...
def fg_MibLLDBInit_Pointer(_Debugger):
	...
	fg_AddSynth(_Debugger, CSynthProvider_TCSharedPointer, "(^|^const )NMib::NStorage::TCSharedPointerSupportWeak<.*>$", True)
	...
	fg_AddSummary(_Debugger, fg_SummaryProvider_TCSharedPointer, "(^|^const )NMib::NStorage::TCSharedPointerSupportWeak<.*>$", True)
