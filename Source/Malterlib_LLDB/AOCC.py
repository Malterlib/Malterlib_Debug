...
class CSynthProvider_TCIntNoAutoConvert(CSynthProvider_Common):
	...
	def update(self):
		...
		try:
			if self.m_ValueObjectType.GetPointeeType().IsPointerType():
				return
			self.m_NumExtraChildren = 0
			self.m_Value = self.m_ValueObjectDeref.GetChildMemberWithName('m_Data')
...
def fg_MibLLDBInit_AOCC(_Debugger):
	
	# TCIntNoAutoConvertAggregate
	fg_AddSynth(_Debugger, CSynthProvider_TCIntNoAutoConvert, "(^|^const )TCIntNoAutoConvert<.*>$", True)
	fg_AddSynth(_Debugger, CSynthProvider_TCIntNoAutoConvert, "(^|^const )TCIntNoAutoConvertAggregate<.*>$", True)
	fg_AddSummary(_Debugger, fg_SummaryProvider_IteratorCommon, "(^|^const )TCIntNoAutoConvert<.*>$", True)
	fg_AddSummary(_Debugger, fg_SummaryProvider_IteratorCommon, "(^|^const )TCIntNoAutoConvertAggregate<.*>$", True)
